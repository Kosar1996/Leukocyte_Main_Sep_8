...
cfg.solid.endothelium.etaE = 0;
cfg.solid.endothelium.useViscoelasticEndothelium = false;
...
%% DIRECT NODAL STRESS vs LAME (apples-to-apples: etaE=0 above, so this
% run IS pure elastic -- no elastic/viscous confound. This is the
% decisive check for whether load-stepping + the fallback-tol fix
% actually restored correct elastic equilibrium.
...
fprintf('\n=== PURE-ELASTIC (etaE=0) LOAD-STEPPED SOLVE vs LAME, same z-band ===\n');
fprintf('median FE elastic sigma_rr        = %.4f Pa\n', median(stressElasticOnly.sigma_rr(nodesBand)));
fprintf('median analytical (Lame) sigma_rr = %.4f Pa\n', median(sigma_rr_analytical(rNodesBand)));
fprintf('ratio FE/analytical = %.4f\n', ...
    median(stressElasticOnly.sigma_rr(nodesBand)) / median(sigma_rr_analytical(rNodesBand)));
