...
%% GLOBAL FORCE BALANCE CHECK: total reaction at the fixed base MUST equal
% total applied force at the interface, for ANY correctly-equilibrated
% solve -- an exact identity independent of material model, mesh quality,
% or locking. This isolates whether the bug is in the SOLVE/EQUILIBRIUM
% itself (imbalance found) or narrower, in stress reporting only (balance
% holds, but reported stress still looks wrong).
ndof = size(meshE.nodes,1)*2;

% Total applied force (radial component) at the interface
...
% Total internal force (= reaction, since Fint=Fext at converged eq'm) at
% the fixed base nodes -- compute Fint at uEcorr and sum radial DOFs there.
[Fint, ~] = assemble_finite_def_axisym(meshE, uEcorr, par);
totalReactionRadial = sum(Fint(2*baseE-1));

fprintf('\n=== GLOBAL FORCE BALANCE CHECK ===\n');
fprintf('Total applied radial force at interface   = %.6e N\n', totalAppliedRadial);
fprintf('Total internal(reaction) radial force at base = %.6e N\n', totalReactionRadial);
fprintf('Ratio (reaction/applied) = %.4f  (should be close to -1.0 for balance, opposite sign convention)\n', ...
    totalReactionRadial / totalAppliedRadial);

% Also do a mid-domain "cut" check: sum radial Fint over a ring of nodes
% at roughly the SAME z-band as our stress sampling (z in [1,3] um), to
% see if the LOCAL force-balance (not just global base reaction) is
% consistent -- sum Fint over ALL free-body nodes below some z cutoff link
% is more complex for a full mesh, so just report base vs interface for now.

fprintf('\nSMOKE_TEST_STATUS: OK\n');

%% WHOLE-MESH FORCE BALANCE: sum(Fint) - sum(Fext) over ALL dofs, radial
% and axial separately. If this is genuinely nonzero (not just "missing
% from my baseE-only reaction sum"), it's an unambiguous, mesh-wide force
% imbalance -- the Newton solve's own residual check only looks at FREE
% dofs and is blind to this.
FextFull = zeros(ndof,1);
[FextFull, ~] = apply_interface_traction(meshE, uEcorr, FextFull, interfaceE, traction);
[FintFull, ~] = assemble_finite_def_axisym(meshE, uEcorr, par);

sumFint_r = sum(FintFull(1:2:end));
sumFext_r = sum(FextFull(1:2:end));
sumFint_z = sum(FintFull(2:2:end));
sumFext_z = sum(FextFull(2:2:end));

fprintf('\n=== WHOLE-MESH FORCE BALANCE (all dofs, free+fixed) ===\n');
fprintf('sum(Fint radial) = %.6e N,  sum(Fext radial) = %.6e N,  diff = %.6e N\n', ...
    sumFint_r, sumFext_r, sumFint_r - sumFext_r);
fprintf('sum(Fint axial)  = %.6e N,  sum(Fext axial)  = %.6e N,  diff = %.6e N\n', ...
    sumFint_z, sumFext_z, sumFint_z - sumFext_z);
fprintf('relative imbalance (radial) = %.4f%%\n', 100*abs(sumFint_r-sumFext_r)/max(abs(sumFext_r),1e-30));

[fixDofsCheck, ~] = solid_support_conditions(baseE, par.supportE);
freeDofsCheck = setdiff((1:ndof).', unique(fixDofsCheck(:)));
Rfree = FintFull(freeDofsCheck) - FextFull(freeDofsCheck);
fprintf('max |Fint-Fext| over FREE dofs (should be ~solver tol) = %.6e N\n', max(abs(Rfree)));
fprintf('number of free dofs = %d, number of fixed dofs = %d, total = %d\n', ...
    numel(freeDofsCheck), numel(fixDofsCheck), ndof);
