...
fprintf('max |radial displacement| = %.6e m\n', max(abs(radialDU)));
fprintf('max |axial displacement|  = %.6e m\n', max(abs(axialDU)));
fprintf('median r at interface nodes = %.6e m\n', median(meshE.nodes(interfaceE,1)));
fprintf('shell thickness estimate (max r - min r) = %.6e m\n', max(meshE.nodes(:,1)) - min(meshE.nodes(:,1)));
fprintf('Ge = %.4f Pa, Ke = %.4f Pa, applied traction = %.1f Pa\n', par.Ge, par.Ke, knownPressure);
