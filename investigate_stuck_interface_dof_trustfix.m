...
% Trust-region fix test: raise the INITIAL cap to trustUMax (2e-7) instead
% of the default solidTrustU0=2e-8, so the first Newton step isn't
% artificially clipped before the trust region has a chance to grow.
% Applied to both par (endothelium) and parL (leukocyte).
if isfield(par, 'solidTrustUMax') && isfinite(par.solidTrustUMax)
    par.solidTrustU0 = par.solidTrustUMax;
else
    par.solidTrustU0 = 2e-7;
end
parL.solidTrustU0 = par.solidTrustU0;
...
nPasses = 8;
% Forced to 1.0 (no damping) for this variant, to test whether the
% "uniformly tiny, load-independent displacement" pattern found at the
% default relax=0.05 is fully explained by relaxation, or persists even
% undamped (which would mean something else is going on).
relax = 1.0;
...
save('stuck_dof_investigation_trustfix.mat', 'uEHist', 'uLHist', 'tnEHist', 'ttEHist', 'tnLHist', 'ttLHist', ...
    'idsE', 'idsL', 'zE_nodes', 'zL_nodes');
fprintf('\nSaved raw per-pass history to stuck_dof_investigation_trustfix.mat\n');
