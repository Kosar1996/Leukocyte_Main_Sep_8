...
%% Replicate assemble_finite_def_axisym.m's OWN Gauss-point stress
% computation directly (not the recovery function's single-centroid-point
% approach), for elements touching the interface, using the CONVERGED
% uEcorr -- to check whether the TRUE equilibrium stress (as the solve
% itself sees it) is close to -500 Pa (recovery function bug) or also
% close to -53 Pa (deeper equilibrium issue).
cache = meshE.axisymCache;
I3 = eye(3);

% Find elements that include at least one interface node
interfaceSet = false(size(meshE.nodes,1),1);
interfaceSet(interfaceE) = true;
elemHasInterfaceNode = false(meshE.nelem,1);
for e = 1:meshE.nelem
    if any(interfaceSet(meshE.conn(e,:)))
        elemHasInterfaceNode(e) = true;
    end
end
elemList = find(elemHasInterfaceNode);
elemList = elemList(1:min(6,numel(elemList)));

fprintf('\n=== TRUE GAUSS-POINT STRESS (from assemble_finite_def_axisym.m''s own formula) ===\n');
fprintf('%-8s %-6s %-12s %-14s\n', 'elem', 'gp', 'r [um]', 'sigma_rr [Pa]');
for ei = 1:numel(elemList)
    e = elemList(ei);
    dofs = cache.dofs(e,:).';
    ue = uEcorr(dofs);
    conn = meshE.conn(e,:);
