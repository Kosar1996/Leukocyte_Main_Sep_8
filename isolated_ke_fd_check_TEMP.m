...
%% FINITE-DIFFERENCE CHECK OF Kext (apply_interface_traction's tangent)
% Standard FEM verification: perturb one DOF, recompute Fext, compare the
% finite-difference derivative against the analytical Kext column.
ndof = size(meshE.nodes,1)*2;
u0 = uE_pre;  % use the converged-ish state as the linearization point... 
% actually use uEcorr (the converged solution) since that's where Newton
% was actually using Kext during the solve.
u0 = uEcorr;

Fext0 = zeros(ndof,1);
[Fext0, Kext0] = apply_interface_traction(meshE, u0, Fext0, interfaceE, traction);

% Pick a few interface DOFs to check (radial DOFs of a few interface nodes)
zInterfaceAll = meshE.nodes(interfaceE,2);
flatMask = abs(meshE.nodes(interfaceE,1) - 2.5e-6) < 1e-9;
flatNodes = interfaceE(flatMask);
testNodes = flatNodes(round(linspace(3, numel(flatNodes)-2, 4)));

eps_fd = 1e-12;
fprintf('\n=== Kext FINITE-DIFFERENCE CHECK ===\n');
fprintf('%-10s %-16s %-16s %-16s %-10s\n', 'dof', 'analytical Kext', 'FD approx', 'abs diff', 'rel diff');
for ni = 1:numel(testNodes)
    nd = testNodes(ni);
    dofR = 2*nd-1;
    uPert = u0; uPert(dofR) = uPert(dofR) + eps_fd;
    FextPert = zeros(ndof,1);
    [FextPert, ~] = apply_interface_traction(meshE, uPert, FextPert, interfaceE, traction);
    fdCol = (FextPert - Fext0) / eps_fd;

    % Compare the SAME dof's own diagonal-ish entry (dFext(dofR)/du(dofR))
    anaVal = full(Kext0(dofR, dofR));
    fdVal = fdCol(dofR);
    absdiff = abs(anaVal - fdVal);
    reldiff = absdiff / max(abs(fdVal), 1e-20);
    fprintf('%-10d %-16.6e %-16.6e %-16.6e %-10.4f\n', dofR, anaVal, fdVal, absdiff, reldiff);

    % Also check the norm of the full column difference (not just diagonal)
    anaCol = full(Kext0(:,dofR));
    colDiff = norm(anaCol - fdCol);
    colNorm = norm(fdCol);
    fprintf('    full column: |analytical-FD| = %.6e, |FD| = %.6e, rel = %.4f\n', ...
        colDiff, colNorm, colDiff/max(colNorm,1e-20));
end

fprintf('\nSMOKE_TEST_STATUS: OK\n');

%% FINITE-DIFFERENCE CHECK OF Ke (assemble_finite_def_axisym's internal
% tangent stiffness) -- same technique, now checking the material/
% deformation stiffness instead of the follower-load stiffness.
[Fint0, Ke0] = assemble_finite_def_axisym(meshE, u0, par);

fprintf('\n=== Ke (internal stiffness) FINITE-DIFFERENCE CHECK ===\n');
fprintf('%-10s %-16s %-16s %-16s %-10s\n', 'dof', 'analytical Ke', 'FD approx', 'abs diff', 'rel diff');
for ni = 1:numel(testNodes)
    nd = testNodes(ni);
    dofR = 2*nd-1;
    uPert = u0; uPert(dofR) = uPert(dofR) + eps_fd;
    [FintPert, ~] = assemble_finite_def_axisym(meshE, uPert, par);
    fdCol = (FintPert - Fint0) / eps_fd;

    anaVal = full(Ke0(dofR, dofR));
    fdVal = fdCol(dofR);
    absdiff = abs(anaVal - fdVal);
    reldiff = absdiff / max(abs(fdVal), 1e-20);
    fprintf('%-10d %-16.6e %-16.6e %-16.6e %-10.4f\n', dofR, anaVal, fdVal, absdiff, reldiff);

    anaCol = full(Ke0(:,dofR));
    colDiff = norm(anaCol - fdCol);
    colNorm = norm(fdCol);
    fprintf('    full column: |analytical-FD| = %.6e, |FD| = %.6e, rel = %.4f\n', ...
        colDiff, colNorm, colDiff/max(colNorm,1e-20));
