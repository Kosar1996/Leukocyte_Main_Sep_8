%% REBUILD_STEP2_TABLE_NEW_DEFINITION
% Rebuilds the "page 1" style Step 1&2 table (r, z, sigma_tangent solid/fluid,
% sigma_normal solid/fluid, P, %mismatch normal/tangential) using CURRENT
% correct methodology (unified domain, fixed position-aware interpolation)

%   %mismatch = |solid - fluid| / |fluid| * 100
...
fprintf('================ UPDATED STEP 1&2 TABLE (new local-fluid-relative %%mismatch) ================\n');
fprintf('%4s %10s %8s %12s %12s %12s %12s %10s %10s %10s\n', ...
    'Pt','r[um]','z[um]','tang(solid)','tang(fluid)','norm(solid)','norm(fluid)','P[Pa]','MismN%','MismT%');
...
for idx = 1:2
    ...
    fprintf('%4s %10.4f %8.4f %12.2f %12.2f %12.2f %12.2f %10.2f %9.1f%% %9.1f%%\n', ...
        labelsL{idx}, rWallL*1e6, z*1e6, ttS, ttF, tnS, tnF, pF, pctN, pctT);
end
for idx = 1:2
    z = zPoints(idx);
    ...
    pctT = 100*abs(ttS-ttF)/abs(ttF);
    fprintf('%4s %10.4f %8.4f %12.2f %12.2f %12.2f %12.2f %10.2f %9.1f%% %9.1f%%\n', ...
        labelsE{idx}, rWallE*1e6, z*1e6, ttS, ttF, tnS, tnF, pF, pctN, pctT);
end
