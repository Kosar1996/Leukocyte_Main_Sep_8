...
%% HAND-CALCULATION CHECK: pick a flat-region interface node pair, compute
% the EXPECTED equivalent nodal radial force by hand from first principles,
% compare directly against what apply_interface_traction.m actually
% produces for the SAME nodes.
zInterfaceAll = meshE.nodes(interfaceE,2);
flatMask = abs(meshE.nodes(interfaceE,1) - 2.5e-6) < 1e-9;  % exactly r=2.5 (flat section)
flatNodes = interfaceE(flatMask);
flatZ = meshE.nodes(flatNodes,2);
[flatZsorted, ord] = sort(flatZ);
flatNodesSorted = flatNodes(ord);

% pick two adjacent flat-region nodes, well away from any end
midIdx = round(numel(flatNodesSorted)/2);
n1 = flatNodesSorted(midIdx);
n2 = flatNodesSorted(midIdx+1);
r1 = meshE.nodes(n1,1); z1 = meshE.nodes(n1,2);
r2 = meshE.nodes(n2,1); z2 = meshE.nodes(n2,2);
fprintf('\n=== HAND-CALC CHECK ===\n');
fprintf('node n1=%d: r=%.6e z=%.6e\n', n1, r1, z1);
fprintf('node n2=%d: r=%.6e z=%.6e\n', n2, r2, z2);
L = sqrt((r2-r1)^2 + (z2-z1)^2);
fprintf('segment length L = %.6e m (should be ~pure dz since flat, r1=r2)\n', L);

% Hand calc: for a UNIFORM traction tn=-500 over this segment, pure radial
% (n_hat=[1;0] since dr=0), 2-point Gauss quadrature, linear shape funcs.
% Nodal force (radial DOF) for node 1 from this ONE segment:
%   fe(1) = integral( N1(xi) * tn * n_hat_r * 2*pi*r(xi) * (L/2) ) dxi, xi in [-1,1]
xi_gp = [-1,1]/sqrt(3); w_gp = [1,1];
fe1_radial_manual = 0;
fe2_radial_manual = 0;
for gg = 1:2
    xi = xi_gp(gg); wg = w_gp(gg);
    N1v = 0.5*(1-xi); N2v = 0.5*(1+xi);
    r_gp = N1v*r1 + N2v*r2;
    fac = (2*pi*r_gp) * (L/2) * wg;
    tn_val = knownPressure;  % uniform
    fe1_radial_manual = fe1_radial_manual + N1v * tn_val * fac;
    fe2_radial_manual = fe2_radial_manual + N2v * tn_val * fac;
end
fprintf('Hand-calc contribution to n1 radial force from THIS segment = %.6e N\n', fe1_radial_manual);
fprintf('Hand-calc contribution to n2 radial force from THIS segment = %.6e N\n', fe2_radial_manual);
fprintf('(n1, n2 each also get a contribution from their OTHER adjacent segment,\n so full nodal force = sum of two segments; both segments are essentially\n identical here since traction and r are uniform, so full force ~= 2x one segment for interior nodes)\n');

% Now get the ACTUAL Fext MATLAB computes for these nodes' radial DOFs.
ndof = size(meshE.nodes,1)*2;
FextActual = zeros(ndof,1);
[FextActual, ~] = apply_interface_traction(meshE, uE_pre, FextActual, interfaceE, traction);
fprintf('\nACTUAL Fext(n1 radial) = %.6e N\n', FextActual(2*n1-1));
fprintf('ACTUAL Fext(n2 radial) = %.6e N\n', FextActual(2*n2-1));
fprintf('Expected (2x one-segment hand-calc, interior node, both segments equal) = %.6e N\n', 2*fe1_radial_manual);
fprintf('Ratio actual/expected for n1 = %.4f\n', FextActual(2*n1-1) / (2*fe1_radial_manual));
