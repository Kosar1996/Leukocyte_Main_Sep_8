function diag = softlube_plot_select_global1D_domain(cfg, out,plotstep, opts)
...
%   diag = softlube_plot_global1D_domain(cfg, out);
    ...
    if ~isempty(out) && isstruct(out) && isfield(out, 'state')
        if isfield(out.stateHist{plotstep}, 'uE') && ~isempty(out.stateHist{plotstep}.uE)
            uE = out.stateHist{plotstep}.uE(:);
        end
        if isfield(out.stateHist{plotstep}, 'uL') && ~isempty(out.stateHist{plotstep}.uL)
