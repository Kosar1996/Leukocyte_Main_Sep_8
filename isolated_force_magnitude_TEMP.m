...
%% ISOLATED FORCE-MAGNITUDE CHECK: compute what apply_interface_traction
% produces for a known uniform pressure, and compare directly against the
% analytical total force expected from the axisymmetric line-integral
% formula: F_total = integral( traction * 2*pi*r ) ds along the boundary.
...
Fext = zeros(ndof,1);
[Fext, ~] = apply_interface_traction(meshE, uE_pre, Fext, interfaceE, traction);

% Total radial force (sum of odd DOFs = radial components), should be ~0
% by axisymmetry for a closed surface, but let's check the NORMAL-direction
% magnitude via the interface node radial forces directly, and separately
% compute the analytical expected total |force| along the interface as a
% sanity check on magnitude (not sign/direction, which follower-load
% geometry complicates).
FextAtInterface = Fext(2*interfaceE-1); % radial DOF forces at interface nodes
fprintf('sum |Fext radial| at interface nodes = %.6e N\n', sum(abs(FextAtInterface)));
fprintf('max |Fext radial| at interface nodes = %.6e N\n', max(abs(FextAtInterface)));

% Analytical estimate: for each interface segment, force ~ traction * 2*pi*r*dz
rInterface = meshE.nodes(interfaceE, 1);
[zSorted, order] = sort(zInterface);
rSorted = rInterface(order);
segLen = diff(zSorted);
rMid = (rSorted(1:end-1) + rSorted(2:end))/2;
analyticalForcePerSeg = abs(knownPressure) * 2*pi*rMid .* segLen;
fprintf('analytical total force estimate (sum of |pressure|*2*pi*r*dz per segment) = %.6e N\n', sum(analyticalForcePerSeg));
fprintf('ratio (sum|Fext radial| / analytical) = %.4f\n', sum(abs(FextAtInterface)) / sum(analyticalForcePerSeg));
