...
I3 = eye(3);
rInterfaceRef = min(meshE.nodes(interfaceE,1));  % 2.5e-6, the flat-section radius

% Find elements whose Gauss points are actually close to the true interface
% radius (within 100 nm), not just elements sharing a corner node with it.
fprintf('\n=== TRUE Gauss-point stress, elements with gp r within 100nm of interface (r=%.4f um) ===\n', rInterfaceRef*1e6);
...
for e = 1:meshE.nelem
    ...
    for g = 1:cache.ngp
        ...
        zg = N * (meshE.nodes(conn,2));
        fprintf('%-8d %-6d %-12.6f %-14.4f  (z=%.4f um)\n', e, g, rg*1e6, T(1,1), zg*1e6);
        nPrinted = nPrinted + 1;
    end
end
fprintf('total gauss points found within 100nm of interface: %d\n', nPrinted);
