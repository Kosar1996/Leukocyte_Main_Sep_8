...
%% Check whether diagonal scaling actually improves conditioning for THIS
% system, and whether MATLAB's sparse backslash was already effectively
% doing this internally.
u0 = uEcorr;
ndof = size(meshE.nodes,1)*2;
Fext0 = zeros(ndof,1);
[Fext0, ~] = apply_interface_traction(meshE, u0, Fext0, interfaceE, traction);
[Fint0, Ke0] = assemble_finite_def_axisym(meshE, u0, par);
R0 = Fint0 - Fext0;
[fixDofsC, ~] = solid_support_conditions(baseE, par.supportE);
freeC = setdiff((1:ndof).', unique(fixDofsC(:)));
Ktot0 = Ke0;  % no Kext contribution needed for condition check on Ke alone; use full assembly incl Kext for realism:
FextForK = zeros(ndof,1);
[~, Kext0] = apply_interface_traction(meshE, u0, FextForK, interfaceE, traction);
Ktot0 = Ke0 - Kext0;
Kff0 = Ktot0(freeC, freeC);

dscale = sqrt(abs(full(diag(Kff0))));
dscale(dscale < eps(class(dscale)) | ~isfinite(dscale)) = 1;
Dinv = spdiags(1./dscale, 0, numel(dscale), numel(dscale));
Kscaled0 = Dinv * Kff0 * Dinv;

fprintf('\n=== CONDITION NUMBER CHECK ===\n');
fprintf('size(Kff) = %d x %d\n', size(Kff0,1), size(Kff0,2));
c1 = condest(Kff0);
c2 = condest(Kscaled0);
fprintf('condest(Kff, unscaled)  = %.6e\n', c1);
fprintf('condest(Kff, diag-scaled) = %.6e\n', c2);
fprintf('improvement factor = %.4f\n', c1/c2);
